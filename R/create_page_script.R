...
create_page_script <- function(source_file, R_folder = "R/", overwrite = FALSE) {
  ...
  # Error check
  if (!exists("id")) stop("Missing the `id` object in the `source_file`.")
  if (!exists("var_left"))
  ...
  if (file.exists(new_file) && !overwrite) return(file.show(new_file))
  ...
  if (length(var_left) == 1) {
    ...
    template <- gsub(paste0('var_left <- reactive\\(paste\\(`__var_left__`, ',
                            'time\\(\\), sep = "_"\\)\\)'),
  ...
  template <- gsub("more_args = reactive\\(c\\(\\)\\)",
                   paste0("more_args = reactive(c(",
                          paste0(more_args, collapse = ",\n"),
                          "))"),
                   template)

  # Add all uis
    template <- gsub("susSidebarWidgets\\(\\)",
                     paste0("susSidebarWidgets(\n",
                            paste0(uis, collapse = ",\n"),
                            ")"),
                     template)
  ...
  # Message
  message(paste0("Make sure the page is refered in the id column of the ",
                 "`modules` data.frame."))
