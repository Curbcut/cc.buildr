...
rev_geocode_QC <- function(point_sf) {
  ...
  link <-
    paste0("https://servicescarto.mern.gouv.qc.ca/pes/rest/services/Territoi",
  ...
  if (!is.null(out$error)) return(NA_character_)
...
rev_geocode_BC <- function(point_sf) {
  ...
  link <-
    paste0("https://geocoder.api.gov.bc.ca/sites/nearest.json?point=",
           x, ",", y)

  out <- httr::content(httr::GET(link))

  if (is.null(out$properties$fullAddress)) return(NA_character_)
...
rev_geocode_OSM <- function(point_sf) {
  ...
  link <- paste0("https://nominatim.openstreetmap.org/reverse?format=json&lat=",
                 y, "&lon=", x, "&addressdetails=1")

  out <- httr::content(httr::GET(link))

  if (is.null(out$address)) return(NA_character_)
  ...
  third <-
    out |> (\(out) {
      ...
      if (!is.null(out$address$town)) return(out$address$town)
      if (!is.null(out$address$village)) return(out$address$village)
      if (!is.null(out$address$suburb)) return(out$address$suburb)
      if (!is.null(out$address$region)) return(out$address$region)
    })()

  second <-
    out |> (\(out) {
      if (is.null(out$address$road)) return(third)
      return(paste(out$address$road, third, sep = ", "))
    })()

  name <-
    out |> (\(out) {
      if (is.null(out$address$house_number)) return(second)
