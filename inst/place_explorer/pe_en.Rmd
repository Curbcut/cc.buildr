...
curbcut::ordinal_form <- function(x) {
...
contents <- lapply(all_themes, \(t) {
  ...
  cards <- lapply(all_vars, \(var) {
    ...
    sum(grepl(var, names(scale_df)))
    # If the column does not exist, return nothing
    if (sum(grepl(var, names(scale_df))) == 0) return(NULL)
    ...
    list(box = bslib::value_box(
      ...
      value = curbcut::s_sentence(paste0(curbcut::ordinal_form(percentile), " percentile")),
      showcase = myImage,
