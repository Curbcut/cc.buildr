...
contents <- lapply(all_themes, \(t) {
    ...
    cards <- lapply(vars, \(var) {
        scale_df <- params$scale_df
        scale_df <- sf::st_drop_geometry(scale_df)
        
        # Necessary so `curbcut` functions work
        ...
        title <- params$variables$var_title[params$variables$var_code == var]
    ...
    pv <- which(names(scale_df) == parent_string)
    id_to_households <- 1:which(names(scale_df) == "households")
    
    to_assign_global_env <- scale_df[unique(c(id_to_households, pv))]
    assign(df, to_assign_global_env, envir = .GlobalEnv)
    ...
    main_card_boxes_outliers <- cards_ordered[sapply(cards_ordered, `[[`, "ranking") > 0.40]
...
main_card_outliers <- lapply(contents, `[[`, "main_card_boxes_outliers")[sapply(contents, `[[`, "ranking") > 0.35]
    ...
    link <- 
        if (!is.na(ind$link_module)) {
            out <- paste0("/?tb=", ind$link_module, 
                          "&zm=", params$map_zoom, 
                          "&crds=", paste0(round(params$map_loc[1], digits = 2), ";", round(params$map_loc[2], digits = 2)), 
