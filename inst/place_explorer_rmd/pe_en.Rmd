...
contents <- lapply(all_themes, \(t) {
    ...
    cards <- lapply(vars, \(var) {
        ...
        type <- unlist(params$variables$type[var == params$variables$var_code])
...
main_card_outliers <- do.call(bslib::layout_column_wrap, c(list(width = 1/2), unname(main_card_outliers)))
...
main_card_normal <- do.call(bslib::layout_column_wrap, c(list(width = 1/2), unname(main_card_normal)))
...
if (!ind$percentile == "No data.") {
    link <- 
        if (!is.na(ind$link_module)) {
            out <- paste0("/?tb=", ind$link_module, "&zm=", params$map_zoom, "&lat=", round(params$map_loc[2], digits = 4), "&lon=", round(params$map_loc[1], digits = 4), "&s_id=", params$select_id)
...
main_card <- bslib::nav(
    ...
    htmltools::br(),
    if (length(main_card_outliers) > 0) htmltools::h3("What makes this area unique?"),
    if (length(main_card_outliers) > 0) bslib::card_body_fill(main_card_outliers),
    if (length(main_card_normal) > 0) htmltools::h3("What makes this area similar to others?"),
    if (length(main_card_normal) > 0) bslib::card_body_fill(main_card_normal)
